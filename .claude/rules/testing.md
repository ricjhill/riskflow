...
- **Scale (`tests/scale/`)** — regression tests for performance/concurrency properties that broke before. Each test pins a specific incident: event loop starvation under load (#198), `/jobs` latency budget at N items (#195). Use real Redis via testcontainers, run against the wired-up app, and assert on latency budgets or non-blocking behaviour — not throughput. These are not load tests (use `tests/load/` for Locust), they are invariants that must keep holding as the codebase changes.
