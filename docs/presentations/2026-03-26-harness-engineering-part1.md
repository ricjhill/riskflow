# Harness Engineering Part 1: Building the Constraints
...
*Walk through the harness layout on screen.*
...
Harness engineering is about asking the right questions in the right order:
...
3. **Are the tests strong enough?** → Testing rules + parametrize + boundary values
4. **Does it actually run?** → Smoke tests found a deprecated SLM model
5. **Is the process efficient?** → Automated routine steps, kept human approval for risky ones
6. **How do we prevent regressions?** → Security scanning + CI hooks
